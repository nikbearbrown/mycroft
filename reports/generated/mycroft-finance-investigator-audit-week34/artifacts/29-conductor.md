...
### Step 8 — Package and Verify the Evidence Chain

- Labor: AI [PA] [TO], followed by human adequacy review
- AI task: Confirm that the validation, baseline run, open review request,
  evaluation cases, scenario plan, raw files, and verified files form one exact
  hash-bound chain. Package the data, specifications, implementation, tests,
  logs, and human views; then independently recompute every bundled hash.
- Human task: Receive the bundle checksum through a trusted channel and judge
  whether the included evidence and tests are adequate.
- Handoff condition: The manifest, human review view, and every packaged
  artifact match their recorded SHA-256; release remains blocked.
