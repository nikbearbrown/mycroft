...
2. Step name: Alpha Vantage Market Data. Labor: AI with Human gate.
   Script called: `scripts/ingest/mycroft-forecasting-agent__alpha-vantage-market-data.py`
...
3. Step name: FinBert. Labor: AI with Human gate.
   Script called: `scripts/tools/mycroft-forecasting-agent__finbert.py`
   Input: approved upstream output or sample fixture.
   Output: local handoff JSON fields: action, approved_for_live_action:false, input_refs, output_refs, flags, live_call_performed.
   Where output goes: logs/.
4. Step name: Send email. Labor: AI with Human gate.
   Script called: `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__send-email.py`
   Input: approved upstream output or sample fixture.
...
5. Step name: Send email1. Labor: AI with Human gate.
   Script called: `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__send-email1.py`
   Input: approved upstream output or sample fixture.
   Output: markdown report sections: run summary, source status, validation results, flags, typed TODOs, decision recommendation.
   Where output goes: reports/generated/.
6. Step name: Respond to Webhook. Labor: AI with Human gate.
   Script called: `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__respond-to-webhook.py`
   Input: approved upstream output or sample fixture.
   Output: markdown report sections: run summary, source status, validation results, flags, typed TODOs, decision recommendation.
   Where output goes: reports/generated/.
7. Step name: Webhook. Labor: AI with Human gate.
   Script called: `scripts/tools/mycroft-forecasting-agent__webhook.py`
   Input: approved upstream output or sample fixture.
   Output: local handoff JSON fields: action, approved_for_live_action:false, input_refs, output_refs, flags, live_call_performed.
   Where output goes: logs/.
8. Step name: Respond to Webhook1. Labor: AI with Human gate.
   Script called: `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__respond-to-webhook1.py`
   Input: approved upstream output or sample fixture.
   Output: markdown report sections: run summary, source status, validation results, flags, typed TODOs, decision recommendation.
   Where output goes: reports/generated/.
9. Step name: Produce human report. Labor: AI with Human review.
   Script called: `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__produce-human-report.py`
...
|---|---|---|
| Alpha Vantage Market Data | `snickerdoodle run mycroft-forecasting-agent --step alpha-vantage-market-data` | `--sample` |
| FinBert | `snickerdoodle run mycroft-forecasting-agent --step finbert` | `--no-write` |
| Send email | `snickerdoodle run mycroft-forecasting-agent --step send-email` | `--no-write` |
| Send email1 | `snickerdoodle run mycroft-forecasting-agent --step send-email1` | `--no-write` |
| Respond to Webhook | `snickerdoodle run mycroft-forecasting-agent --step respond-to-webhook` | `--no-write` |
| Webhook | `snickerdoodle run mycroft-forecasting-agent --step webhook` | `--no-write` |
| Respond to Webhook1 | `snickerdoodle run mycroft-forecasting-agent --step respond-to-webhook1` | `--no-write` |
| Produce human report | `snickerdoodle run mycroft-forecasting-agent --step produce-human-report` | `--no-write` |

### Gate Commands

| Gate | CLI Command |
|---|---|
| Gate 1 - source/input readiness | `snickerdoodle gate mycroft-forecasting-agent --gate 1 --decision approve --note "..."` |
| Gate 2 - sample run | `snickerdoodle gate mycroft-forecasting-agent --gate 2 --decision approve --note "..."` |
| Gate 3 - report contract | `snickerdoodle gate mycroft-forecasting-agent --gate 3 --decision approve --note "..."` |

### Script Locations

| Step | Script Path | Layer |
|---|---|---|
| Alpha Vantage Market Data | `scripts/ingest/mycroft-forecasting-agent__alpha-vantage-market-data.py` | ingest |
| FinBert | `scripts/tools/mycroft-forecasting-agent__finbert.py` | tool |
| Send email | `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__send-email.py` | tool |
| Send email1 | `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__send-email1.py` | tool |
| Respond to Webhook | `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__respond-to-webhook.py` | tool |
| Webhook | `scripts/tools/mycroft-forecasting-agent__webhook.py` | tool |
| Respond to Webhook1 | `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__respond-to-webhook1.py` | tool |
| Produce human report | `[TODO: DEV] Create or map script path: scripts/tools/mycroft-forecasting-agent__produce-human-report.py` | tool |
