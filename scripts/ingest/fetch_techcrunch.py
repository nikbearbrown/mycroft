"""Purpose: Fetch the TechCrunch artificial-intelligence RSS feed.
Input: Optional JSON object with url; default is the n8n TechCrunch feed URL.
...
def fetch_techcrunch(config: dict[str, Any] | None = None, timeout: int = 20) -> dict[str, Any]:
    """Purpose: Fetch the TechCrunch artificial-intelligence RSS feed.
    ...
    Recipe: recipes/workflow-a-extract-store-raw-data.md
    """
    url = (config or {}).get("url") or "https://techcrunch.com/category/artificial-intelligence/feed/"
    with urllib.request.urlopen(url, timeout=timeout) as response:
        raw = response.read().decode("utf-8", errors="replace")
    return {"source_name": "TechCrunch", "url": url, "raw": raw, "bytes": len(raw)}


if __name__ == "__main__":
    payload = load_input({})
    try:
        result = fetch_techcrunch(payload["data"])
    except Exception as exc:
        result = {"source_name": "TechCrunch", "error": str(exc)}
