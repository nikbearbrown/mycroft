"""Purpose: Fetch the Hacker News AI/LLM RSS feed.
Input: Optional JSON object with url; default is the n8n HNRSS query URL.
...
def fetch_hackernews(config: dict[str, Any] | None = None, timeout: int = 20) -> dict[str, Any]:
    """Purpose: Fetch the Hacker News AI/LLM RSS feed.
    ...
    Recipe: recipes/workflow-a-extract-store-raw-data.md
    """
    url = (config or {}).get("url") or "https://hnrss.org/frontpage?q=AI+LLM+machine+learning&count=20"
    with urllib.request.urlopen(url, timeout=timeout) as response:
        raw = response.read().decode("utf-8", errors="replace")
    return {"source_name": "HackerNews", "url": url, "raw": raw, "bytes": len(raw)}


if __name__ == "__main__":
    payload = load_input({})
    try:
        result = fetch_hackernews(payload["data"])
    except Exception as exc:
        result = {"source_name": "HackerNews", "error": str(exc)}
