"""Purpose: Convert verified articles into collection B document records.
Input: List of verified article records from data/verified/news-monitoring-agent/.
Output: Chunk-ready document records for collection news_articles_b.
...
def load_documents_b(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Purpose: Convert verified articles into collection B document records.
    Input: List of verified article records.
    Output: Document records for collection news_articles_b.
    Side effects: None except optional caller-managed file write.
    Idempotent: Yes; same input yields same output.
    Recipe: recipes/news-monitoring-agent.md
    """
    return build_documents(items, "news_articles_b")


if __name__ == "__main__":
    payload = load_json_input([{"title": "x", "content": "body", "url": "u"}])
    emit_json(load_documents_b(payload["data"]), payload["output"])
