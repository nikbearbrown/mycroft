"""Purpose: Parse VentureBeat RSS text into raw_intelligence records.
Input: JSON object with raw feed text from data/raw/aeo-workflow-a/.
Output: Parsed VentureBeat records for data/verified/aeo-workflow-a/.
...
def parse_venturebeat(payload: dict[str, Any] | str) -> list[dict[str, Any]]:
    """Purpose: Parse VentureBeat RSS text into raw_intelligence records.
    Input: JSON object with raw feed text.
    Output: Parsed VentureBeat records.
    ...
    return parse_rss_or_atom(str(raw), "VentureBeat", "news", "ai")


if __name__ == "__main__":
    sample = {"raw": "<rss><channel><item><title>AI news</title><link>https://example.com/v</link><description>Body</description></item></channel></rss>"}
    payload = load_input(sample)
    emit(parse_venturebeat(payload["data"]), payload["output"])
