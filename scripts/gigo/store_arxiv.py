"""Purpose: Prepare ArXiv rows for raw_intelligence storage.
Input: Parsed ArXiv records from data/verified/aeo-workflow-a/.
...
def store_arxiv(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Purpose: Prepare ArXiv rows for raw_intelligence storage.
    Input: Parsed ArXiv records.
    ...
    Recipe: recipes/workflow-a-extract-store-raw-data.md
    """
    return prepare_store_rows(items, "ArXiv")


if __name__ == "__main__":
    payload = load_input([{"title": "AI", "url": "https://arxiv.org/abs/1", "raw_content": "Body"}])
    emit(store_arxiv(payload["data"]), payload["output"])
