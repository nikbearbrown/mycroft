...
**Agentic workflow demo** — A working FastAPI application in which four specialized agents run in a hybrid sequence: a KYC agent and an OSINT agent fire in parallel, a quantitative agent runs afterward once both complete, and a reasoning/report agent synthesizes the results — but only if none of the first three agents returns a blocking signal. A blocking signal (a KYC block, a high-severity OSINT finding, or a failed financial ratio) halts the pipeline and routes the assembled evidence directly to a human reviewer, skipping memo synthesis. Absent a block, the completed memo is routed to the correct human approval tier — Senior Credit Officer, Credit Committee, or Executive Credit Committee — by loan size and risk, before any credit decision record can be written. See the `case-study-workflows/agentic-credit-workflow/` directory.

**Developer architecture reference** — The `case-study-workflows/agentic-credit-workflow/README.md` covers every component in the pipeline: agent specifications, the data source adapter pattern, the human gate implementation, the audit trail schema, and a quick-start guide.
