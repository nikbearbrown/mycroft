...
| `theses/STM.md` | Ch.1, updated in Ch.2 | The claim-audit: every claim in the thesis, tagged verified/inferred/unsupported/taste/needs-a-source. Ch.2 added real evidence that upgraded one claim's tag. |
...
| `theses/human-card-STM.md` | Ch.4 | The human's job only: purpose, caveats, open questions, and an empty decision block. Reads the agent recipe's output; never fills its own decision. |
